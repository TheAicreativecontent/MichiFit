...
VARIANTES = {
    "gris":   {"luz": (0.12, 0.52), "sat": 0.03, "rubor": 0.99},
    "blanco": {"luz": (0.56, 0.94), "sat": 0.035, "rubor": 0.95},
}
...
def distanciaTono(a, b):
    """Cuantos grados hay entre dos tonos, POR EL CAMINO CORTO.

    El tono es un circulo: 355 grados esta a 5 de 0, no a 355. La
    primera version comparaba con `hd < 16` y por eso los mofletes
    salian como anillos huecos: su rosa tiene pixeles a los dos lados
    del cero —unos en 4 grados, otros en 355— y solo se cogian los de
    un lado. Los del otro se tenian como pelaje, y quedaba un borde
    gris rodeando el centro rosa."""
    d = abs(a - b) % 360
    return min(d, 360 - d)
...
def esMejilla(hd, l, s):
    ...
    return distanciaTono(hd, TONO_ROSA) <= MARGEN_ROSA and l > 0.45 and s > 0.30


# Una mancha rosa mas pequena que esto no es un moflete: es un pixel
# suelto del antialiasing entre el pelaje y el contorno. Sin este filtro
# aparecen puntitos rosas por toda la cara y el pecho.
MANCHA_MINIMA = 12


def manchasDeRosa(im):
    """Los pixeles rosas que forman MANCHA, no los sueltos.

    Los mofletes, la nariz, la lengua y el interior de las orejas son
    zonas compactas. Un pixel rosa perdido en mitad del lomo es ruido, y
    si se conserva queda una pena de purpurina sobre el gato gris."""
    W, H = im.size
    px = im.load()
    candidatos = set()
    for y in range(H):
        for x in range(W):
            ...
            if l > LIMITE_OSCURO and esMejilla(h * 360, l, s):
                candidatos.add((x, y))
    ...
    for p in candidatos:
        ...
        while pila:
            ...
            for d in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                n = (x + d[0], y + d[1])
                if n in candidatos and n not in vistos:
                    vistos.add(n)
                    pila.append(n)
        if len(grupo) >= MANCHA_MINIMA:
            buenos.update(grupo)
    return buenos


def retocadoAMano(origen, destino):
    """¿Alguien ha tocado el archivo generado despues de generarlo?
    ...
    if not os.path.exists(destino):
        return False
    return os.path.getmtime(destino) > os.path.getmtime(origen) + 2
...
def main():
    ...
    for color, cfg in VARIANTES.items():
        ...
        for ruta in poses:
            ...
            if not forzar and retocadoAMano(ruta, destino):
                print("  %-20s RETOCADO A MANO, se respeta" % nombre)
                respetados += 1
                continue
    ...
    print("\n%d generados, %d respetados por estar retocados a mano."
          % (hechos, respetados))
    if respetados:
        print("Para rehacerlos igualmente:  python pixel/tenir_michi.py --forzar")
    print("Se eligen en la app: Ajustes > Tu michi > Tu gato.")
